...
import main.project.server.guesthouse.stub.CityStub;
...
public class GuestHouseTotalTests {
    ...
    void getGuestHouseMainFilterTest1() {
        ...
        Member registerMember = registerMember(MemberStub.getEmptyMember().setAndReturn("테스터"));
        City city = City.City(1L);

        GuestHouse guestHouse = GuestHouse.builder()
                .guestHouseName("1번 케이스")
                .member(registerMember)
                .city(city)
                .guestHouseTag("|감성|").build();
    ...
    void getGuestHouseMainFilterTest2() {
        ...
        //given
        Member member = Member.builder().memberId("테스터").build();
        registerMember(member);

        City city = City.City(1L);
        String reserveStart = "2023-02-12";
        String reserveEnd = "2023-02-19";
        PageRequest pageRequest = PageRequest.of(0, 10);

        GuestHouse guestHouse = GuestHouse.builder().guestHouseId(null).guestHouseName("2번 케이스").member(member).city(city).guestHouseTag("|감성|").build();
        Room room = Room.builder().roomId(null).roomName("1-1룸").guestHouse(guestHouse).roomStatus(RoomStatus.ROOM_ENABLE).build();
        guestHouse.setRooms(List.of(room));
        guestHouseRepository.save(guestHouse);


        RoomReservation alreadyReservation = RoomReservation.builder()
        ...
        roomReservationRepository.save(alreadyReservation);


        //where
        Page<GuestHouse> guestHouseByFilter = guestHouseRepository.findGuestHouseByFilter(city.getCityId().intValue(), "%", reserveStart, reserveEnd, pageRequest);


        //then

        Assertions.assertEquals(true,roomReservationRepository.findAll().contains(alreadyReservation)); //예약이 잘 등록되어 있는지
        ...
        for (GuestHouse g : guestHouseByFilter) {
    ...
    void getGuestHouseMainFilterTest3() {
        ...
        GuestHouse guestHouse = GuestHouse.builder().guestHouseId(null).guestHouseName("3번 케이스").member(member).city(city).guestHouseTag("|감성|").build();
        Room room = Room.builder().roomId(null).roomName("1-1룸").guestHouse(guestHouse).roomStatus(RoomStatus.ROOM_ENABLE).build();
        guestHouse.setRooms(List.of(room));
        guestHouseRepository.save(guestHouse);
    ...
    void getGuestHouseMainFilterTest4() {
        ...
        GuestHouse guestHouse = GuestHouse.builder().guestHouseId(null).guestHouseName("4번 케이스").member(member).city(city).guestHouseTag("|감성|").build();
        Room room = Room.builder().roomId(null).roomName("1-1룸").guestHouse(guestHouse).roomStatus(RoomStatus.ROOM_ENABLE).build();
        guestHouse.setRooms(List.of(room));
        guestHouseRepository.save(guestHouse);
    ...
    void getGuestHouseMainFilterTest5() {
        ...
        GuestHouse guestHouse = GuestHouse.builder().guestHouseId(null).guestHouseName("5번 케이스").member(member).city(city).guestHouseTag("|감성|").build();
        Room room = Room.builder().roomId(null).roomName("1-1룸").guestHouse(guestHouse).roomStatus(RoomStatus.ROOM_ENABLE).build();
        guestHouse.setRooms(List.of(room));
        guestHouseRepository.save(guestHouse);
    ...
    void getGuestHouseMainFilterTest6() {
        ...
        GuestHouse guestHouse = GuestHouse.builder().guestHouseId(null).guestHouseName("6번 케이스").member(member).city(city).guestHouseTag("|감성|").build();
        Room room = Room.builder().roomId(null).roomName("1-1룸").guestHouse(guestHouse).roomStatus(RoomStatus.ROOM_ENABLE).build();
        guestHouse.setRooms(List.of(room));
        guestHouseRepository.save(guestHouse);
